...
## PHASE 9A DECISIONS (2026-09-17)

### 2026-09-17 — CORRECTION: "moves no clinical threshold" is too strong; the imaging refutation is WEAKENED within site and CONFIRMED cross-site
**What was wrong.** The Phase 7 DECISION LOG entry of 2026-09-12 states that from controlled capture the inversion "reaches the MARGINAL band only - screening bands at best - **does not beat site + sex + age, and moves no clinical threshold**". The first clause stands and is unchanged: on MAE the CNN (1.301) does not beat site + sex + age (1.273). **The second clause is contradicted by measurement and is corrected.**
...
**What the measurement found.** Phase 9A evaluated the referral decision rather than the g/dL estimate, at an operating point pre-declared before running and chosen on training folds only. Pooled site-mixed CV, n=216, 91 anaemic: at matched sensitivity (~0.90) the image reduces the referral burden against site+sex+age by **+0.096 specificity, CI [+0.017,+0.179]** (CNN) and **+0.192, CI [+0.114,+0.276]** (three mean palpebral CIELAB numbers plus a ridge). The colour model's gain **clears the >= 0.10 margin this phase pre-declared as clinically meaningful**, with a CI excluding zero; referral rate 0.519 against the baseline's 0.634 at the same detection rate. AUROC over the same baseline: CNN +0.059 CI [+0.010,+0.110], colour +0.057 CI [+0.005,+0.110], both excluding zero. A clinical decision threshold therefore **does** move.
**Why the arm verdict nevertheless does not change, and on what grounds.** No model earns USEFUL. Two reasons, of very different strength, and they must not be conflated:
- **A knife-edge that this data cannot resolve.** The models clearing a margin miss the  pre-declared 0.90 sensitivity floor by **0.010** (0.890 against 0.900; bootstrap CI  [0.822, 0.950]), and the CNN misses the margin itself by **0.004**. With 91 anaemic  subjects these boundaries are inside the noise. **They are reported as unresolved, not as  decisive**, and nothing was adjusted to land on either side of them.
- **A clean failure that is decisive.** Cross-site, with the operating point chosen on the  training site: `italy_to_india` **sensitivity 0.397, flagging 27 of 68 anaemic subjects**  (the +1.55 g/dL bias carries predictions above the train-site cut); `india_to_italy`  **specificity 0.418**, below the floor. Robust across all three seeds (sensitivity  0.250-0.426; no seed meets both floors in either direction). Section 3 of CLAUDE.md:  *single-site accuracy numbers are considered worthless.*
**And the within-site effect is site-driven, which bounds it further.** Decomposing by site: the CNN's AUROC advantage over age+sex is **+0.012, CI [-0.105,+0.141] in India** (n=95, prevalence 0.716) and **+0.445, CI [+0.292,+0.593] in Italy** (n=121, prevalence 0.190). The pooled advantage comes from one of the two sites.
**Decision.** The imaging arm's recorded status (**not deployable**) is retained, and the **stated reason is changed**: it is the cross-site collapse, not the absence of a clinically meaningful within-site effect. Section 2 of CLAUDE.md is amended to say so. The phrase "moves no clinical threshold" must not be reused. **"CLOSED, FINAL" for the imaging arm stands as a decision not to build further, and is now explicitly NOT a claim that the image carries no decision-relevant information - it demonstrably does, within site.**
**Alternatives considered.** (1) Leaving the Phase 7 wording and reporting Phase 9A as confirmatory - rejected: the wording is contradicted by a pre-declared test, and burying that is the failure mode this project's own audit harness exists to catch. (2) Relaxing the 0.90 sensitivity floor or the 0.10 margin so a verdict lands cleanly either way - rejected outright; the criteria were committed before the run and are not adjustable afterwards in either direction. (3) Declaring the arm USEFUL on the colour model's cleared margin - rejected: it misses the sensitivity floor and fails cross-site.
**Consequences.**
- A reviewer's objection that the project refuted the imaging arm on a metric the product  would not use is **partly correct**, and the answer is now measured rather than asserted.
- The strongest within-site screening model is **not the CNN**: three mean CIELAB numbers  match or beat it on every screening axis (AUROC 0.874 vs 0.875; specificity at matched  sensitivity 0.752 vs 0.656). Any future imaging work should start there, not from a CNN.
- `reports/phase9a_screening_metrics.md` leads with the model-versus-baseline comparison.  The correction is also recorded in `docs/archive/corrections.md`.

### 2026-09-17 — Phase 7's PPG "sensitivity 0.00" is a plug-in artefact, and is now labelled as one
**Decision.** The Phase 7 figure is **reproduced exactly and retained** — sensitivity 0.000, flagging none of the 18 anaemic subjects — with the rule that produces it named in every statement of it: *refer if the predicted Hb is below the diagnostic threshold*.
**Why the label matters.** A regression model shrinks its predictions toward the mean, so a plug-in rule at a diagnostic threshold is systematically insensitive; 0.000 measures the rule as much as the model. At an operating point chosen for screening the same PPG models reach **sensitivity 0.889**. Reporting 0.000 without that context would overstate the negative result, in the same way that reporting 0.889 without the specificity beside it would overstate the positive one.
**It does not rescue the arm.** At that operating point the selected deep model runs at specificity **0.098**, referring **90.1% of everyone**, with **NNS 15.8 against 14.0 for referring everybody** — worse than referring the whole cohort. Its AUROC advantage over demographics is **-0.000, CI [-0.195,+0.189]**, and at matched sensitivity it is **significantly worse** than demographics (specificity **-0.090, CI [-0.142,-0.042]**). The PPG verdict is **CONFIRMED and strengthened**.
**Consequences.** `reports/statistical_vs_clinical.md` records "sensitivity 0.00 (prevalence 7%)"; that number is correct and stays, and the plug-in qualification travels with it from here on.

### 2026-09-17 — Two defects in this phase's own comparison code, found and fixed before any number was reported
**Decision.** Recorded rather than quietly corrected, because both would have biased the result toward the conclusion already on record.
**1. The symmetric margin test did not exist.** `compare()` computed both of its branches with `nested_calls_matched_specificity`, so the branch labelled *matched sensitivity* matched specificity a second time and then compared specificities that were **equal by construction**. The pre-declaration says a specificity gain at matched sensitivity "counts equally", so the test that was missing is precisely the one the imaging arm goes on to pass (+0.192, CI [+0.114,+0.276]). Fixed by adding `screening.nested_calls_matched_sensitivity`, which holds the reference's train-fold sensitivity and maximises specificity.
**2. `verdict()` consulted only one of the two margins.** It read the matched-specificity difference alone, so a model buying specificity rather than sensitivity could never clear the margin however large the gain. Fixed to accept **either** direction, as pre-declared.
**Effect on the verdicts: none, and that was checked rather than assumed.** After both fixes every verdict label is unchanged, because the models that clear a margin miss the 0.90 sensitivity floor independently. The fixes changed what is *reported*, not what is *concluded* — without them the +0.192 specificity gain, which is the whole basis of the correction above, would never have been computed.
**Lesson, matching the one logged 2026-09-12 about a constant in a report generator.** A comparison that is wired to the wrong quantity still returns plausible numbers. What exposed it was reading a result that could not be true — a baseline with AUROC 0.464 apparently achieving sensitivity 0.870 at specificity 0.765 — and not accepting it.

### 2026-09-20 — Phase 9B: the literature audit is widened to seven full texts, with rules fixed before scoring
**Decision.** Every full text available to the project — seven PDFs in `data/raw/literature/` — is read in full and scored on six criteria: demographic baseline; split level *and* whether augmentation preceded the split; cross-site or cross-device evaluation; deduplication *and* an explicit leakage statement; pooled versus per-site reporting; dataset used. The scoring is data in `hemosight.audit.literature`, rendered into `reports/literature_gap.md` §9B and `data/interim/phase9b/literature_audit.json`, with the Phase 5 table retained unchanged above it.
**Rules fixed before any paper was scored.** (1) Five answer categories — YES, NOT REPORTED, UNKNOWN, NOT APPLICABLE, FULL TEXT UNAVAILABLE — never collapsed; a qualifier after a dash records what the paper states. (2) **No paper is ever recorded as failing a check.** This audit measures what is *reported*; "not reported" and "audited and failed" are different findings, and conflating them is the error this project criticises. The single measured NO (Phase 1, byte-identical duplicates in the Ghana collections) stays on the Phase 5 dataset rows and is not transferred to any paper row — a test enforces that no paper value begins NO or FAIL. (3) Single-site status is recorded as NOT APPLICABLE for the cross-site and per-site criteria, not as a failure. (4) Two of the brief's criteria are recorded as the two things they actually ask: split level and augmentation order separately; deduplication and a leakage statement separately.
**Rationale.** The project's loudest rhetorical claim rested on 5 sources with most fields UNKNOWN and one measured NO — too thin to support it. Widening the base is the only honest way to find out whether the claim survives, and fixing the categories first is what stops the widening from becoming a search for confirmation.
**Alternatives considered.** Scoring from abstracts — rejected; every criterion lives in Methods or Results. Seeking the four unobtained dataset papers now — not done in this phase; they are recorded as FULL TEXT UNAVAILABLE with "not sought" stated, so the obtainability count is honest about what was and was not attempted. Treating Asare et al. 2023 as the "paper" behind the Ghana dataset rows and rewriting those rows — rejected; the dataset rows describe the released data and the Phase 1 measurements, and the paper is scored on its own row.
**Consequences.** `reports/literature_gap.md` gains §9B (obtainability, counts, the full table with evidence, the overlap consequence, what the counts support); `reports/final_results.md` §6 carries the Phase 9B counts; `scripts/phase9b_literature_audit.py` is a reproduce stage; `tests/test_phase9b.py` pins the categories and the framing. One dataset identity could not be settled from the text (Sehar et al. 2025: 710 downloaded images cited as Eyes-Defy-Anemia but matching the CP-AnemiC / Ghana collection in count and class split) and is recorded as UNKNOWN rather than resolved by inference.

### 2026-09-20 — Phase 9B: the claim "a literature that frequently omits it" is NARROWED to what the counts support
**Decision.** The sentence in CLAUDE.md section 2 — *"a demographic baseline that exposes how easily an apparently working estimator is really a sex classifier. That last point alone is a contribution to a literature that frequently omits it"* — is narrowed. The new wording: *none of the seven anaemia-estimation papers whose full text this project read reports a demographics-only baseline; this is a statement about those seven, from a convenience sample, not about the field.* The original wording is retained in place under a banner and copied verbatim to `docs/archive/superseded_claims.md` §6.
**What the counts are.** Demographic baseline: **0 of 7** report one — a complete count, no UNKNOWN, because every text was read. Per-site reporting: **0 of the 3** multi-site or multi-source papers (Lin 2025, Asare 2023, Sehar 2025) report a per-site result. Deduplication: 0 of 7 mention one; a leakage-avoidance statement: 3 of 7. Split unit stated: 3 of 7 (two subject-level, one random over augmented images); augmentation order stated: 3 of 7 (two before the split, one after). Cross-site or cross-device result: 1 of 7 (a new prospective cohort, same institution). Obtainability: **12 identified, 7 obtained**.
**Why narrow rather than keep.** "Frequently" is a rate with the field as its denominator. The denominator here is seven accessible papers with no search strategy and no inclusion criteria. 0 of 7 is *consistent with* the old sentence and cannot *establish* it; a claim must follow the counts, and the counts support a statement about seven papers. The same rule was applied in Phase 5 when the sample was five, and widening the sample by two does not change what kind of statement it licenses — only a systematic review would.
**What the audit does license, and only as a worked example.** All three multi-site papers read pool their sites, and this project has shown on its own model what a pooled number can conceal (Phase 9A: AUROC 0.875 pooled; 0.688 India, 0.909 Italy; the entire advantage over demographics from one site). One of the three (Lin 2025) pools the very two sites this project decomposed. That is a specific, checkable point for the write-up — not a rate.
**Alternatives considered.** Keeping the sentence on the grounds that 0 of 7 is strong — rejected, because the sample is a convenience sample and the sentence is about the field. Replacing it with a claim about pooled reporting — rejected as a headline, because n = 3; retained as a worked example.
**Consequences.** CLAUDE.md section 2 and CURRENT STATE updated; `reports/final_results.md` §6 and `reports/literature_gap.md` §9B.4 carry the narrowed wording; `tests/test_phase9b.py::test_claim_was_narrowed_and_the_original_wording_kept` fails if the unbannered original returns. **The write-up must not use "frequently omits" or any field-level rate.** No verdict on any arm changes.

### 2026-09-20 — Phase 9C: the gate result is reported as a distribution, and the propagation is tied to the machinery it propagates
**Decision.** The Phase 3 gate number is re-reported as a distribution over a declared prior on the 14 tissue parameters the forward model held fixed. The forward model is **not** rebuilt: `hemosight.simulation.uncertainty` exposes the fixed parameters of the existing `forward.layered_reflectance` / `diffuse_reflectance` path as a vector theta, and the gate's own perturbation generator, Hb grid (4-18), LUT (2-24 step 0.05) and trial count (240 per Hb) are reused verbatim.
**The validity condition, declared before running and enforced in code.** The perturbation banks are regenerated in the original RNG order (seed 20260911, Phase 3 order [1.0, 2.0, 3.935]; Phase 7 order [3.456, 1.981, 1.062]) so that theta = nominal must return the recorded point estimates. It does: **3.892987 / 3.470951 / 1.039592, reproduced to 7e-13, 7.6e-13 and 3.5e-13.** The script aborts and writes `valid: false` if that check fails, and a test asserts the parameterised reflectance equals `forward.layered_reflectance` at nominal to 1e-12. Without this the intervals would be about a different model than the one that closed the arm.
**Two settings, reported separately and never pooled.** *Self-consistent* (the inversion LUT, the truth and the forward model all at theta) is what section 2 of the report and every headline interval use, because it isolates the question "does the verdict survive not knowing these parameters". *Mismatch* (LUT at nominal, truth at theta) is the Phase 3 Task 4 question, gives much larger numbers (median 9.64 g/dL at the gate residual), and is reported in its own section with a statement that it is not evidence about the gate.
**Rationale.** A reviewer can say the number that decided the project inherits uncertainty the project documented but never propagated. That is a fair objection, and the only honest answer is the measurement. Fixing the prior, the design and the interpretation rule in CLAUDE.md before the first run is what stops the answer from being chosen.
**Alternatives considered.** Sweeping one parameter at a time (what Phase 3 Task 4 did) — retained as a secondary table but rejected as the headline, because the forward model is strongly non-additive and one-at-a-time sweeps pass through degenerate corners the joint sample almost never visits (bvf_epi = 0 gives MAE 10.7 g/dL and an 8.09 g/dL swing that the Sobol totals correctly demote). Installing SALib — rejected, a 60-line Saltelli/Jansen implementation with bootstrap CIs avoids a new dependency for one phase. Widening the prior after seeing that the studio condition was fragile — **explicitly refused**; the pre-declaration forbids it.
**Consequences.** `src/hemosight/simulation/uncertainty.py`, `scripts/phase9c_uncertainty.py` (+ `--reuse-design`), `scripts/phase9c_report.py`, `reports/phase9c_uncertainty.md`, `data/interim/phase9c/`, `tests/test_phase9c.py`, two reproduce stages. Every recorded appearance of the gate number now carries its interval with the point estimate retained: CLAUDE.md (CURRENT STATE, section 2 rows 3 and 8, the Phase 3 phase-table row), `final_results.md`, `phase3_simulation.md`, `phase7.md`, and the front end's case study and pre-registration pages. **Nothing was deleted.**

### 2026-09-20 — Phase 9C: the imaging refutation is ROBUST to parameter uncertainty; the controlled-capture result is FRAGILE
**Decision.** Applying the pre-declared rule — robust if the entire 95% interval falls on one side of the 2.0 g/dL threshold, fragile if it spans it — the imaging arm's refutation is restated as **robust to parameter uncertainty**, and Phase 7's studio result is restated as **fragile**. Both directions were reported whichever way they fell; no range was adjusted.
**The numbers.** At the Phase 3 residual (3.935 dE2000): median **4.67 g/dL**, 95% **[2.37, 8.02]**, 99.5% of the prior above 2.0, point estimate 3.893 at percentile 32. At the uncontrolled 3.456: median **4.28**, 95% **[2.09, 7.90]**, 98.4% above 2.0. Both intervals lie **entirely above** 2.0. Only **10 of 2,048** draws at the gate residual fall below 2.0, each requiring 5 of 14 parameters to sit simultaneously in the outer quarter of their own generous ranges, and **the single most favourable draw still lands at 1.75 g/dL — MARGINAL, never VIABLE.** No physically plausible combination rescues uncontrolled capture.
**What is weakened, stated plainly.** The studio condition (1.062 dE2000, recorded 1.040 g/dL MARGINAL, 0.04 above the VIABLE line) becomes median **1.52**, 95% **[0.60, 4.90]**: **27.1% of the prior VIABLE, 35.9% MARGINAL, 37.0% NOT RECOVERABLE.** Under parameter uncertainty it does not have a band; it has a distribution across all three. Its MARGINAL label is a property of the nominal parameter choice as much as of the capture condition. **It must never again be quoted as 1.04 alone.** This weakens a result the project already called MARGINAL, not the central refutation — but it is the one imaging number that sat near a favourable band, and the write-up carries the interval.
**Why this is not a softening of the negative result.** The refutation got stronger: it now holds across a 14-dimensional prior in which 13 parameters are unsourced and the ranges were chosen to be generous. The fragile item is the one that pointed the *other* way.
**Consequences.** Section 2 row 8 of CLAUDE.md gains "and **FRAGILE** under parameter uncertainty"; `phase7.md`'s outcome-B verdict carries the interval at the exact sentence that reads the boundary. The recorded MARGINAL band is not withdrawn — the point estimate is correct for the nominal parameters — it is qualified.

### 2026-09-20 — Phase 9C: the sensitivity ranking does not confirm that melanin dominates, and that is reported rather than smoothed
**Decision.** Phase 3 recorded melanin as "the model's most consequential assumption" (a 0.005 volume fraction shifts recovered Hb by +9.6 g/dL) and `optical_data.melanin_absorption` carries a banner saying so. Phase 9C's Sobol analysis does **not** rank it first in the self-consistent setting, and the report says so instead of confirming the expectation.
**The measurement.** Total-effect indices at the 3.935 residual: **deep-layer contribution weight 0.303**, epithelial blood volume fraction 0.284, **melanin volume fraction 0.258**, stromal BVF 0.187, StO2 0.119. At the studio residual melanin **is** first (0.319) — it matters most where capture noise is smallest. The melanin *shape* parameters (exponent, amplitude) score ~0 because the gate's nominal melanin fraction is 0, so they have nothing to act on.
**Reconciliation, not contradiction.** Phase 3's +9.6 g/dL is a **mismatch** result — a model calibrated at zero melanin meeting a pigmented subject — and it is reproduced here (mismatch median 9.64 g/dL at the gate residual, 7.65 conditional on melanin <= 0.005). Both are true of different questions. The banner in `optical_data.py` stays as written because its instruction ("sweep it, do not fix it") is exactly what this phase did.
**The uncomfortable part, recorded because it is the point of the exercise.** The single largest driver at the uncontrolled residual is the factor **0.6** by which `forward.layered_reflectance` down-weights deeper layers — a modelling convention with **no citation at all**, not a tissue property. The second is the epithelial blood volume fraction, also unsourced. The result is most sensitive to the parts of the model nobody sourced.
**The recommendation, therefore split rather than headline-grabbing.** A measured conjunctival melanin absorption curve (amplitude and exponent, with inter-individual spread) is the most valuable future **spectroscopic** measurement and the one that matters for N5 skin-tone bias. It is **not** the most valuable measurement overall at the uncontrolled residual, where two unsourced structural quantities matter as much, and the larger of those costs nothing to fix because it is a convention.
**Mechanism, measured.** Across 400 sampled parameter vectors the rank correlation between the gate MAE and the chromaticity distance the model places between Hb 4 and 18 g/dL is **-0.96**, and that span varies over an order of magnitude across the prior (median 0.0674, 95% [0.0125, 0.1414]). The gate MAE is colour noise over colour signal per g/dL — Phase 3's own stated mechanism, now measured across the prior rather than at one point.

### 2026-09-20 — CORRECTION: Efron 2009 was obtained and does NOT contain the thicknesses attributed to it; the VERIFICATION REQUIRED banner is kept and strengthened
**What it corrects.** `simulation/constants.py` cites [5] Efron N., Al-Dossari M., Pritchard N., *In vivo confocal microscopy of the palpebral conjunctiva and tarsal plate*, Optom. Vis. Sci. 86(11):E1303-8, 2009 for the palpebral layer thicknesses, and the banner said the values "could not be verified against the source" because the full text was subscription-only (recorded 2026-09-12, Phase 6.5).
**What was done.** Phase 9C Task 4 sought both outstanding sources with network access. The paper itself remains closed (Europe PMC `isOpenAccess: N`, `inEPMC: N`), but **the same study is a chapter of the open-access QUT master's thesis it came from** — Al Dossari M., *Confocal microscopic examination of the conjunctiva*, QUT ePrints 18316 — same 11 healthy participants, same HRT3/RCM instrument, same tissue, abstract matching the published one. It was downloaded (`data/raw/literature/Al_Dossari_thesis_QUT18316.pdf`, untracked) and read in full.
**The correction.** The source **contains no thickness in micrometres for any palpebral layer.** Verbatim: the epithelium "appears to be only two or three cell-layers deep"; the stroma is "thin"; the tarsal plate is "dark and amorphous". Its only thickness measurement, **32.9 +/- 1.1 um, is of the BULBAR conjunctival epithelium** — a different tissue, in the companion bulbar chapter. It also states that "it is therefore not possible to determine the overall thickness of the conjunctiva using this (or any other) technique with any degree of certainty". **No blood volume fraction appears anywhere in it.** The previous wording was too generous: the values were not merely unverified, the cited source does not contain them.
**Zhivov 2006 [6]** (The Ocular Surface 4(2):81-93, PMID 16681079) was **not obtained**: Europe PMC not open access and not in EPMC, OpenAlex `oa_status: closed` with `any_repository_has_fulltext: false`, no author manuscript located. Its abstract contains no numeric thickness.
**Consequence.** The banner is **KEPT and strengthened**, from "could not be verified against the source" to "the source was read and does not contain them", for the stromal thickness (200 um), the tarsal-plate thickness (800 um) and all three blood volume fractions. The citations are **not removed**: [5] remains the source of the qualitative layer structure the model uses, which is what it actually supports. The 32 um epithelial thickness keeps its lifted status on the strength of Li et al. 2015 (OCT, 34.0 +/- 5.8 um, palpebral) — now understood to be the *only* independent support it ever had, and the reason it is the single parameter Phase 9C labels SOURCED. **No verdict changes:** Phase 9C measures stroma thickness at rank 7 of 14 and tarsal-plate thickness at rank 14, so narrowing those priors would have moved the interval very little. The parameters that dominate are not the ones the banner is about. Also recorded in `docs/archive/corrections.md`.

### 2026-09-20 — Phase 9D: the minimum detectable effect is computed; retrospective power is refused
**Decision.** Each arm's negative result is accompanied by the smallest effect its deciding comparison could have detected: **MDE = (z_0.975 + z_power) x SE(paired difference)**, two-sided, alpha 0.05, at 80% and 90% power. **Observed / retrospective / post-hoc power is not computed**, and `hemosight.evaluation.power` deliberately contains no function that would produce it; `tests/test_phase9d.py::test_retrospective_power_is_absent_from_the_module` fails if one is ever added.
**Why refusing it matters more than naming it.** Observed power is a deterministic function of the observed p-value, so it carries no information the p-value does not, and it is low by construction whenever a result is null. In a project whose contribution is a body of negative results, reporting it would hand every one of them a built-in excuse — precisely the kind of self-serving analysis this project criticises elsewhere. `power_at()` does exist, but it takes a **pre-declared** effect size (the clinical yardstick) and is never called with an observed effect; that distinction is the whole point and is stated in the report so a reader can check which analysis was run.
**One comparison per arm, and only the one that decided it.** No power is computed for a comparison the project never made. Imaging regression: image-CNN MAE vs site+sex+age. Imaging screening: specificity gain at matched sensitivity and sensitivity gain at matched specificity, vs site+sex+age. Cross-site: the same, per direction. PPG: deep-model MAE vs sex alone, plus AUROC and specificity vs demographics.
**Everything is paired, because the comparisons are.** Two models scored on the same subjects produce correlated errors; an unpaired SE would be wider than the evidence warrants and would inflate every MDE. For the MAE comparisons the exact paired SE sd(d)/sqrt(n) is used — on the imaging comparison the two models' per-subject errors correlate at **r = 0.62**, making the paired SE **38% smaller** than an unpaired one. For sensitivity and specificity at a nested operating point no closed form applies, so the SE is the SD of the identical 2,000-resample subject-level paired bootstrap Phase 9A used (seed 20260911).
**The yardsticks are ones the project already declared; none is invented here.** Screening: the **0.10** clinically meaningful margin pre-declared in Phase 9A. MAE: the **1.0 g/dL** narrowest WHO severity band (the mild band for men, 10.0-10.9), which is the scale an MAE improvement must reach before it can change a clinical decision. **AUROC has no pre-declared yardstick in this project**, so AUROC MDEs are reported as supporting information and **no arm verdict rests on them** — borrowing the 0.10 margin as an AUROC threshold would have been an invented number.
**Alternatives considered.** Computing observed power as well "for completeness" — rejected for the reason above. Closed-form power formulas for the screening comparisons (e.g. McNemar) — rejected, because the operating point is chosen by nested per-fold selection and the resulting statistic is not the one those formulas describe; the bootstrap SD measures the actual estimator. Pooling the two cross-site directions to gain n — rejected, the pre-declaration forbids it and pooling would hide that the two directions have very differently composed test sets.
**Consequences.** `src/hemosight/evaluation/power.py`, `scripts/phase9d_power.py`, `scripts/phase9d_report.py`, `reports/phase9d_power.md`, `data/interim/phase9d/power.json`, `tests/test_phase9d.py` (14 tests), two reproduce stages. A new limitations subsection in `reports/final_results.md`; CURRENT STATE and a FINAL STATUS block paragraph in CLAUDE.md. The script asserts it reproduces Phase 9A's recorded point differences before reporting anything, so these MDEs describe the comparisons that actually decided the arms rather than re-derived look-alikes. **No recorded verdict changes.**

### 2026-09-20 — Phase 9D: two arms are adequately powered; the project's most load-bearing finding is its least powered comparison
**Decision.** Applying the pre-declared rule — ADEQUATELY POWERED iff MDE(80%) <= the arm's already-declared clinical yardstick — the verdicts are recorded as follows, whichever way they fell.
**ADEQUATELY POWERED.** *Imaging regression* (n=216): MDE **0.181 g/dL** at 80%, 0.209 at 90%, against a 1.0 g/dL yardstick — **100% power** to detect a clinically meaningful improvement. *PPG regression* (n=252, venous HemoCue): MDE **0.137 g/dL**, the best-powered comparison in the project. *PPG specificity* at matched sensitivity: MDE **0.073** against the 0.10 margin, 97% power, because specificity is estimated on the 234 non-anaemic subjects. **For both regression arms the effects too small to detect are also far too small to matter** — 0.18 and 0.14 g/dL move no WHO boundary and no gate band — so those negative results are informative evidence of absence rather than merely inconclusive. That is the symmetric counterpart of Phase 7's statistical-versus-clinical finding.
**UNDERPOWERED, marginally: imaging screening within site.** MDE **0.116** specificity (0.114 sensitivity) against the 0.10 margin, i.e. 16% over, **67% power at the margin**. The precise statement matters: what the design could not reliably detect is an effect sitting *exactly at* the margin. What it could detect, it did — the mean-CIELAB model's observed +0.192 specificity gain is well above its own MDE of 0.113 and Phase 9A recorded it with a CI excluding zero. **This is not a null result resting on thin power.**
**UNDERPOWERED, and this is the uncomfortable one: imaging cross-site.** Per direction, never pooled: **italy→india MDE 0.314** specificity — **3.1x the pre-declared margin, 14% power** — and **india→italy MDE 0.165** (40% power). The cause is composition rather than raw n: italy→india tests on 95 subjects of whom 68 are anaemic, so **specificity is estimated on 27 people**. The cross-site collapse is the result that turns a within-site success into a non-deployable verdict, and it is the least precisely estimated comparison in the project.
