...
# ------------------------------------------------ Phase 8B: the visual system's constraints
FRONT = ROOT / "app" / "frontend" / "src"


def test_colours_are_defined_in_tokens_css_only():
    """Every colour is a CSS custom property in tokens.css (generated by
    scripts/design_tokens_contrast_phase8b.py, where the contrast ratios are measured). No hex colour
    may be hard-coded in a component, page or the stylesheet."""
    hexre = re.compile(r"#[0-9a-fA-F]{3,8}\b")
    for f in list(FRONT.rglob("*.tsx")) + list(FRONT.rglob("*.ts")) + [FRONT / "index.css"]:
        if "data" in f.parts or f.name.endswith(".test.ts"):
            continue
        src = f.read_text(encoding="utf-8")
        hits = [m.group(0) for m in hexre.finditer(src) if not src[max(0, m.start() - 12):m.start()].rstrip().endswith("/*")]
        assert not hits, (f.name, hits[:5])
    assert (FRONT / "tokens.css").exists()
    contrast = json.loads((ROOT / "data" / "interim" / "phase8b" / "contrast.json").read_text(encoding="utf-8")) \
        if (ROOT / "data" / "interim" / "phase8b" / "contrast.json").exists() else None
    if contrast:
        assert contrast["all_aa"], [p for p in contrast["pairs"] if not p["aa"]]


def test_verdicts_never_rely_on_colour_alone():
    ui = (FRONT / "components" / "ui.tsx").read_text(encoding="utf-8")
    assert "function Glyph" in ui and ui.count("<svg") >= 3, "each verdict needs a distinct shape"
    assert "VERDICT_LABEL" in ui


def test_reduced_motion_disables_everything_decorative():
    css = (FRONT / "index.css").read_text(encoding="utf-8")
    assert "@media (prefers-reduced-motion: reduce)" in css
    assert 'html[data-motion="off"]' in css
    for comp in ("Particles.tsx", "Cursor.tsx"):
        src = (FRONT / "components" / comp).read_text(encoding="utf-8")
        assert "if (!enabled) return null;" in src, comp
    cursor = (FRONT / "components" / "Cursor.tsx").read_text(encoding="utf-8")
    assert 'setAttribute("data-cursor", "native")' in cursor and "keydown" in cursor, \
        "the native cursor must be restored and the reticle must hide on keyboard use"
    gs = (FRONT / "motion" / "useGsap.ts").read_text(encoding="utf-8")
    assert 'import("gsap")' in gs, "GSAP must be loaded lazily, only when motion is on"


def test_no_dataset_derived_or_external_imagery():
    """Every visual is generated from public tabulated data or the project's own
    aggregate results: no raster image files ship with the front end, and nothing is
    fetched from an external host."""
    for ext in ("*.png", "*.jpg", "*.jpeg", "*.gif", "*.webp"):
        assert not list(FRONT.rglob(ext)), ext
    for f in FRONT.rglob("*.tsx"):
        src = f.read_text(encoding="utf-8")
        assert "https://" not in src.replace("https://creativecommons.org", ""), f.name
    for name in ("hb_spectra.json", "spectral_band.json", "null_distribution.json", "noise_signal.json", "breakeven.json"):
        d = json.loads((FRONT / "data" / name).read_text(encoding="utf-8"))
        assert "source" in d, name
