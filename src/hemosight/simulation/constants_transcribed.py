...
=============================================================================
 ⚠️  VERIFICATION REQUIRED BEFORE PUBLICATION
=============================================================================
The spectral tables below are TRANSCRIBED from the standard published
compilations named in each citation. They have NOT been read from the primary
data files in this environment, which has no network access and ships no
haemoglobin optical data in any installed package.

They are internally consistency-checked (see `validate_constants()`, which
verifies that the haemoglobin isosbestic points land where the literature says
they do, and that known absorption peaks appear at the right wavelengths). That
catches gross transcription errors but CANNOT certify the values to full
published precision.

Before any published claim, re-download the primary sources and re-run
`validate_constants()` plus `tests/test_phase3.py`. Treat every downstream
number as provisional until then. This limitation is recorded in the RESULTS
LOG and in reports/phase3_simulation.md.
=============================================================================
...
def eps_hbo2(wl: np.ndarray) -> np.ndarray:
    """HbO2 molar extinction, cm^-1/M. Source [1]."""
    return _interp(_HB_TABLE, 1, np.asarray(wl, dtype=float))


def eps_hb(wl: np.ndarray) -> np.ndarray:
    """Deoxy-Hb molar extinction, cm^-1/M. Source [1]."""
    return _interp(_HB_TABLE, 2, np.asarray(wl, dtype=float))
...
def mu_a_melanin(wl: np.ndarray) -> np.ndarray:
    """Melanosome absorption coefficient, cm^-1. Source [3]."""
    return MELANIN_A * np.asarray(wl, dtype=float) ** MELANIN_POWER


def mu_a_water(wl: np.ndarray) -> np.ndarray:
    """Water absorption coefficient, cm^-1. Source [4]."""
    return _interp(_WATER_TABLE, 1, np.asarray(wl, dtype=float))


def mu_s_reduced(wl: np.ndarray, scale: float = 1.0) -> np.ndarray:
    """Reduced scattering coefficient mu_s', cm^-1. Source [2]."""
    wl = np.asarray(wl, dtype=float)
    x = wl / 500.0
    return scale * (SCATTER_A_MIE_CM_INV * x ** (-SCATTER_B_MIE)
                    + SCATTER_A_RAY_CM_INV * x ** (-4.0))
