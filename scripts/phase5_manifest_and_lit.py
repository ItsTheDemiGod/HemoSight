...
paper. Counts are reported; no characterisation beyond what the counts support.
    ...
    .\\.venv\\Scripts\\python.exe scripts\\phase5_manifest_and_lit.py
"""
...
def main() -> int:
    ...
    # ---------------------------------------------------- manifest ----------
    L = ["# Dataset manifest\n\n",
         "Generated by `scripts/phase5_manifest_and_lit.py`. Presence and size are read "
    ...
    counts = {f: {v: sum(1 for p in predictive if p[f] == v)
                  for v in ("YES", "NO", "UNKNOWN")} for f in fields}
    ...
    M.append("| attribute | YES | NO | UNKNOWN |\n| --- | --- | --- | --- |\n")
    for f in fields:
        c = counts[f]
        M.append(f"| {f.replace('_', ' ')} | {c['YES']} | {c['NO']} | {c['UNKNOWN']} |\n")
    ...
    M.append("**NOT supported:** any claim that the wider literature omits these "
             "practices. The sample is 10 sources, chosen because this project used "
