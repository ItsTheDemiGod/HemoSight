...
Three sources of known truth:
  ...
  C. CLEAN REPLICATES. Inputs with no injected fault at all, repeated under independent
    ...
    .\\.venv\\Scripts\\python.exe scripts\\phase6_validate_harness.py
...
def a_phase4_tabular(results: list) -> None:
    ...
    scripts/phase4_gate_b.py rather than reimplemented, so the submission the harness
    audits is the model the phase actually reported on.
    """
    print("\n=== A3-A6  Phase 4 / 4.5: the PPG feature model ===")
    from phase4_gate_b import build_conditions, evaluate
...
def a_phase5_deep_end_to_end(results: list) -> bool:
    ...
    regenerated by scripts/phase6_5_deep_predictions.py (10 seeds + 6 candidates),
    ...
    if not p.exists():
        print("\n=== A8  Phase 5 deep model end to end: SKIPPED (run "
              "scripts/phase6_5_deep_predictions.py) ===")
        return False
...
def main() -> int:
    ...
    summary = {
        ...
        "not_validated_end_to_end": [] if deep_e2e else [
            ...
            "run scripts/phase6_5_deep_predictions.py (GPU, ~7 min) and re-run this."],
