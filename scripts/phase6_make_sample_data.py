...
WHERE EACH FILE GOES, AND WHY IT IS NOT ALL ONE DIRECTORY.
  ...
  data/interim/phase6/sample_images/  PNGs for the duplicate check, plus a zip for the
    ...
    .\\.venv\\Scripts\\python.exe scripts\\phase6_make_sample_data.py
"""
...
from phase6_validate_harness import FAULTS, case_seed, synth  # noqa: E402
...
def main() -> int:
    ...
    (SAMPLE / "manifest.json").write_text(
        json.dumps({"generated_by": "scripts/phase6_make_sample_data.py",
...
def write_readme(written: list[dict], zip_path: Path) -> None:
    L = ["# Sample submissions for HemoSight Audit", "",
         "Generated by `scripts/phase6_make_sample_data.py`. Every file here is "
